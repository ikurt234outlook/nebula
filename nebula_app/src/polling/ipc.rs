...
const NEBULA_SOCKET_ENV: &str = "NEBULA_SOCKET";
...
impl IpcListener {
    pub fn new(
        ...
        // Register socket path as environment variable for `nebula msg`.
        unsafe { env::set_var(NEBULA_SOCKET_ENV, path.as_os_str()) };
        if options.daemon {
            println!("NEBULA_SOCKET={}; export NEBULA_SOCKET", path.display());
        }
...
/// Send a message to the active Nebula socket.
...
fn find_socket(socket_path: Option<PathBuf>) -> IoResult<UnixStream> {
    ...
    if let Ok(path) = env::var(NEBULA_SOCKET_ENV) {
    ...
    for entry in fs::read_dir(socket_dir())?.filter_map(|entry| entry.ok()) {
        ...
        // Skip files that aren't Nebula sockets.
